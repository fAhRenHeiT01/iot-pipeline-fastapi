"""Alert Evaluation Engine for Databricks Lakehouse Gold Layer.

Evaluates critical fleet telemetry anomaly rules against a 10-minute sliding event window:
1. EXCESSIVE_IDLE: Engine on & speed 0 continuously exceeding 10 minutes (600s).
...
5. OVERSPEEDING: Sustained speed > 110 km/h for >= 5 minutes (300s).
6. GHOST_TOWING: Engine off with speed > 5 km/h or rapid GPS coordinate translation.
7. COLD_ENGINE_HARD_ACCEL: High speed (> 60 km/h) or rapid accel while engine temp < 50°C.
8. STUCK_SENSOR_ANOMALY: Zero variance in GPS coordinates or temp across >= 5 moving events.
9. THERMAL_SPIKE_AT_IDLE: Rapid temp rise (+5°C in <= 60s or >= 0.08°C/s) while stationary and idle.
"""
...
def evaluate_vehicle_alerts(
    ...
    if latest_engine_status == 1 and latest_speed == 0.0:
        ...
        if idle_seconds > 600.0:
            active_alerts.append((
                "EXCESSIVE_IDLE",
                f"Vehicle idling continuously for {idle_seconds / 60.0:.1f} "
                f"minutes (> 10m threshold)",
            ))

    # -------------------------------------------------------------------------
    # 2. OVERHEATING_CRITICAL Alert: engine_temp >= 75.0°C continuously for > 60s
    # -------------------------------------------------------------------------
    if latest_temp is not None and latest_temp >= 75.0:
        ...
        if overheat_seconds > 60.0:
            active_alerts.append((
                ...
                f"Engine temperature {latest_temp:.1f}°C exceeded 100°C continuously for "
                f"{overheat_seconds:.0f}s (> 60s threshold)",
            ))
    ...
    if len(parsed_events) >= 2:
        ...
        if 0.0 < delta_t <= 15.0:
            ...
            if recent_accel < -15.0:
                active_alerts.append((
                    "HARSH_BRAKING",
                    f"Harsh braking detected: deceleration {recent_accel:.1f} km/h/s "
                    f"(< -15 km/h/s)",
                ))
    ...
    if latest_speed > 110.0:
        ...
        if overspeed_seconds >= 300.0:
            active_alerts.append((
                "OVERSPEEDING",
                f"Sustained overspeeding at {latest_speed:.1f} km/h for "
                f"{overspeed_seconds / 60.0:.1f} minutes (>= 5m threshold)",
            ))
        ...
        if latest_speed > 5.0:
            ...
            if (
                ...
                if gps_speed_kph > 5.0:
                    active_alerts.append((
                        "GHOST_TOWING",
                        f"Ghost towing/rollaway: rapid GPS displacement {gps_speed_kph:.1f} km/h "
                        f"with engine off",
                    ))

    # -------------------------------------------------------------------------
    # 6. COLD_ENGINE_HARD_ACCEL: Engine temp < 50°C while high speed (> 60 km/h) or rapid accel
    # -------------------------------------------------------------------------
    if latest_temp is not None and latest_temp < 50.0:
        if latest_speed > 60.0 or recent_accel > 12.0:
            active_alerts.append((
                "COLD_ENGINE_HARD_ACCEL",
                f"Hard vehicle stress on cold engine: {latest_speed:.1f} km/h "
                f"(temp {latest_temp:.1f}°C < 50°C)",
            ))
    ...
    if len(moving_events) >= 5:
        ...
        if len(coords) >= 5:
            ...
            if max(lats) == min(lats) and max(lons) == min(lons):
                active_alerts.append((
                    ...
                    f"Stuck GPS coordinates ({lats[0]:.4f}, {lons[0]:.4f}) "
                    f"across {len(coords)} moving events",
                ))
        ...
        if len(temps) >= 5 and max(temps) == min(temps):
            active_alerts.append((
                "STUCK_SENSOR_ANOMALY",
                f"Stuck temperature reading ({temps[0]:.1f}°C) with zero variance "
                f"across {len(temps)} moving events",
            ))

    # -------------------------------------------------------------------------
    # 8. THERMAL_SPIKE_AT_IDLE: Engine temp rising rapidly while stationary
    # -------------------------------------------------------------------------
            ...
            for i in range(len(idle_events)):
                for j in range(i + 1, len(idle_events)):
                    ...
                    if 10.0 <= delta_sec <= 120.0:
                        delta_temp = (
                            float(e_end["engine_temp_c"]) - float(e_start["engine_temp_c"])
                        )
                        rate = delta_temp / delta_sec
                        if (delta_sec <= 60.0 and delta_temp >= 5.0) or (
                            delta_sec >= 20.0 and rate >= 0.08
                        ):
                            spike_detected = True
                            spike_details = (
...
def evaluate_vehicle_alerts_pandas(pdf: Any) -> Any:
    ...
    lat_val = float(latest_state["latitude"]) if latest_state["latitude"] is not None else None
    lon_val = float(latest_state["longitude"]) if latest_state["longitude"] is not None else None
    temp_val = (
        float(latest_state["engine_temp_c"]) if latest_state["engine_temp_c"] is not None else None
    )
    status_val = (
        int(latest_state["engine_status"]) if latest_state["engine_status"] is not None else None
    )
    ...
    if active_alerts:
        for alert_type, alert_details in active_alerts:
            rows.append({
                ...
                "engine_temp_c": temp_val,
                "engine_status": status_val,
